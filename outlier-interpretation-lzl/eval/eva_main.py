...
def main(path):
    ...
    model_name1 = "hbos"
    path1 = "data_od_evaluation/" + data_name + "_gt_" + model_name1 + ".csv" #to get evaluation
    path2 = "data_od_evaluation/" + data_name + "_score_" + model_name1 + ".csv"
    if not (os.path.exists(path1) and os.path.exists(path2)):
        print("OD evaluation model training is processing...")
        evaluation_od.evaluation_od_train(X, y, "00_hurt", model_name1)

    return


if __name__ == '__main__':
    input_root_list = [root + "data/"] #to get data
    runs = 1
